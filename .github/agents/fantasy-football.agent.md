...
**Turn Efficiency:** Every shell/tool call costs a turn, and turns are capped for the whole run across every league. Don't rediscover things already documented in this file (the transactions schema, the `externalId` game-join, the `members`/`owners` name join, lineup slot IDs, etc.) with exploratory `python3 -c "print(...)"` calls each run — trust the documented fields and use them directly. Prefer writing one consolidated python3 script per league that loads `data.json` once and computes everything it needs (winners/losers, superlatives, bench stats, streaks, transactions, primetime matches, etc.) in a single pass, over many small one-off snippets. Only fall back to ad hoc exploration for something genuinely unverifiable from this file (e.g., a league's unusual custom roster slots).
